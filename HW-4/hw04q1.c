/* *
* (Simple student record program with basic operations such as adding, displaying, and saving records)
*
* Completion time : 230 minutes
*
* @author Kipchun (AJH)
* @version 10/27/2024
...
int addSort(char* studentName_input, char* major_input, char* schoolYear_input, unsigned int studentID_input)
{
	...
	if (count == MAX_STUDENTS) { return 2;}		// check if list is full

	for (int i = 0; i < count; i++) {
		if (strcmp(list[i].studentName, studentName_input) == 0) { return 0; }	// check if student already exists
	}
	strcpy(list[count].studentName, studentName_input);
	strcpy(list[count].major, major_input);

	if (strcmp(schoolYear_input, "freshman") == 0) { list[count].schoolYear = freshman; }
	else if (strcmp(schoolYear_input, "sophomore") == 0) { list[count].schoolYear = sophomore; }
	else if (strcmp(schoolYear_input, "junior") == 0) { list[count].schoolYear = junior; }
	else if (strcmp(schoolYear_input, "senior") == 0) { list[count].schoolYear = senior; }

	list[count].studentID = studentID_input;
	count++;

	for (int i = 0; i < count - 1; i++) {	// sort the list in ascending order based on student names (case sensitive)
...
void load(char* fileName)
{
	...
	if (file == NULL) { printf("\nStudent_List.txt not found\n"); return; }	// check if file exists)

	fread(&count, sizeof(count), 1, file);        // read the number of students in the list
	...
	for (int i = 0 ; i < count; i++){
		...
		list[i].schoolYear = (schoolYear)schoolYearValue;
